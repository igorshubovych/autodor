...
Yours.NamefinderLookup = function(lookupMethod, value, wp, map, callback) {
	...
	case Yours.lookupMethod.coordToName:
        ...
        console.log(187, "Yours.NamefinderLookup, case Yours.lookupMethod.coordToName:" );
...
Yours.NominatimLookup = function(lookupMethod, value, wp, map, callback) {
	...
	case Yours.lookupMethod.coordToName:
		if (wp !== undefined) {
			point = wp.lonlat.clone();
			/*newPoint = point.transform(map.projection, map.displayProjection);*/
            newPoint = point.transform( epsg900913, epsg4326 );
...
Yours.NameFinder = function(xml, wp, map) {
    console.log("function Yours.NameFinder(xml, wp, map){");
	if (xml.childNodes.length > 0) {
		if (xml.documentElement.nodeName == "searchresults") {
			...
			if (responseLonLat !== undefined) {
                console.log(377,"Yours.NameFinder(xml, wp, map), if (xml.documentElement.nodeName == 'searchresults') if (responseLonLat !== undefined)");
	...
	if (xml.childNodes.length > 0) {
		...
		case "searchresults":
			...
			if (responseLonLat !== undefined) {
				...
				transformedLonLat = responseLonLat.transform(epsg4326, map.getProjectionObject());
...
Yours.Route = function(map, customRouteCallback, customWaypointCallback) {
	 ...
	 *
	 * map.zoomToMaxExtent();
	 * MyFirstRoute = new Yours.Route(map);
	...
	this.permalink = function() {
        console.log(585,"Yours.Route(map, customRouteCallback, customWaypointCallback){ ... this.permalink = function()");
	...
	*/
	...
	this.draw = function() {
        ...
        console.log((pointsOk)?'Find Route':'Ignore');
        //console.log(points);
        if (pointsOk) {
            this.Segments.push(1);
            Autodor.Routing.getRoute(this.Waypoints);
        }
...
Yours.Waypoint = function(ParentRoute)
{
	...
	var self = this;
    var location = "theme/main/images/waypoints/markers/";
	...
	this.markerUrl = function() {
		switch (this.type) {
			case 'via':
				return location + 'number' + this.position + '.png';
			case 'from':
				return location + 'route-start.png';
			case 'to':
				return location + 'route-stop.png';
			default:
				return location + 'marker-yellow.png';
		}
	}
...
Yours.Segment = function(ParentRoute) {
	...
	this.draw = function() {
        console.log(1181,"Yours.Segment.draw();");
        /*
		...
		*/
        var flonlat= this.Start.lonlat.clone().transform(epsg900913, epsg4326);
		var tlonlat= this.End.lonlat.clone().transform(epsg900913, epsg4326);

        

        addScript('http://routes.cloudmade.com/BC9A493B41014CAABB98F0471D759707/api/0.3/' +
				flonlat.lat + ',' + flonlat.lon + ',' + tlonlat.lat + ',' + tlonlat.lon +
