...
function waypointCreateDOM(waypoint) {
	...
	del_button.attr("src", "theme/main/images/waypoints/del.png");
	...
	via_image.attr("src", "theme/main/images/waypoints/ajax-loader.gif");
...
function yournavigation_init() {
	WindowLocation = String(window.location);

	/* Initialize a Route object from the YourNavigation API */
	MyFirstRoute = new Yours.Route(map, myRouteCallback, updateWaypointCallback);

	initWaypoints();
    return ;
	// Check if a permalink is used
	if (false && location.search.length > 0)
    {
			...
			switch (fields[0]) {
				...
				case 'flon':
					...
					if (value !== 0) {
						...
						MyFirstRoute.selectWaypoint(MyFirstRoute.Start.position);
                        console.log(236);
						MyFirstRoute.updateWaypoint("selected", flonlat.clone().transform(map.displayProjection, map.projection));
					}
					break;
				case 'wlat':
				...
				case 'tlon':
					...
					if (value !== 0) {
                        ...
                        console.log(260);
						MyFirstRoute.updateWaypoint("selected", tlonlat.clone().transform(map.displayProjection, map.projection));
					}
					break;
				case 'v':
				...
				case 'layer':
					switch (fields[1]) {
						case 'cycle':
							map.setBaseLayer(layerCycle);
							break;
							/*
						case 'cn':
							map.setBaseLayer(layerCycleNetworks);
							break;
							*/
						default:
				...
				case 'zlon':
					...
					if (value !== 0) {
						zlonlat.lon = value;
                        console.log(295);
						zlonlat.transform(map.displayProjection, map.projection);
					}
					break;
				case 'zlevel':
...
function myRouteCallback(code, result) {
	...
	switch (code) {
		case Yours.status.routeFinished:
			...
			if (!result.quiet) {
				map.zoomToExtent(MyFirstRoute.Layer.getDataExtent());
			}
			break;
		case Yours.status.starting:
			//alert('starting');
			//message_div.attr("src","theme/main/images/waypoints/images/ajax-loader.gif");
			message_div.html('Calculating route <img src="theme/main/images/waypoints/images/ajax-loader.gif">');
			break;
...
function elementChange(element) {
	if (element.value.length > 0) {
		...
		Yours.NominatimLookup( Yours.lookupMethod.nameToCoord, jQuery.trim(element.value), MyFirstRoute.Selected, map, myCallback);
	}
}
